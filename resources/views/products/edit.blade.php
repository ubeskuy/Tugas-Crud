<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-3">
        ...
        <div class="mt-4" x-data="{ imageUrl: '/storage/{{ $product->foto }}' }">
            <form enctype="multipart/form-data" method="POST" action="{{ route('products.update', $product) }}" class="flex gap-7">
                @csrf   
                @method('PUT')

                <div class="w-1/2">
                    <img :src="imageUrl" class="roundded-md"/>
                </div>
                ...
                <div class="w-1/2">
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-area id="deskripsi" class="block mt-1 w-full" type="text" name="deskripsi" required >{{ $product->deskripsi }}</x-text-area>
